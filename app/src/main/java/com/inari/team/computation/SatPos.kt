...
import com.inari.team.computation.utils.Constants.GM_GAL
import com.inari.team.computation.utils.Constants.GM_GPS
import com.inari.team.computation.utils.Constants.OMEGA_EARTH_DOT
import com.inari.team.computation.utils.checkTime
import kotlin.math.*

fun satPos(tRx: Double, satellite: Satellite, constellation: Int): SatPos {
    ...
    with(satellite) {
        ...
        keplerModel?.let {
            ...
            val n0 = if (constellation == Constants.GPS) sqrt(GM_GPS / a.pow(3)) else sqrt(GM_GAL / a.pow(3))
            val n = n0 + it.deltaN
            var m = it.m0 + n * tk
            m = (m + 2 * PI).rem(2 * PI)
            var e = m
